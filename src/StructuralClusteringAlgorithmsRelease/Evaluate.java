...
public class Evaluate implements Constants {
	...
	public double calculateDirectedModularity(int optFunction) {
		...
		while (itVertex_fr.hasNext()) { // Loop factor initialization
			Op_vertex = itVertex_fr.next();
		...
		while (itVertex_fr.hasNext()) { // first loop
			vertex_fr = itVertex_fr.next();

			loop_cnt++;

			if (loop_cnt % 1000 == 0) {
				System.out.println("Loop_cnt:" + loop_cnt);
			}
			...
			while (itVertex_to.hasNext()) { // second loop
				vertex_to = itVertex_to.next();
				...
				if (vertexClust_to <= 0 || vertexClust_fr != vertexClust_to) continue;

				//Set out_neighbors = vertex_fr.getNeighborhood(OUT);

				// outDeg >0, if left any out Degree to process
				if (vertex_fr.isNeighbor(vertex_to.getLabel(), OUT) && outDeg > 0) {
	...
	public double calculateHomogeneity() {
		...
		TreeMap<Integer, Double> clusterTotalHomogeneity = new TreeMap<>();
		TreeMap<Integer, Integer> clusterTotalSize = new TreeMap<>();
	...
	*/
	...
	public double calculateDensity() {
		...
		TreeMap<Integer, Integer> edgesWithinCluster = new TreeMap<>();
		TreeMap<Integer, TreeSet> verticesWithinCluster = new TreeMap<>();
		Vertex vertexA, vertexB;
		Edge edge;
			...
			if (clusterA >= 0 && clusterB >= 0) { // if they are both non-outliers
				...
				if (clusterA == clusterB) { // intra-cluster edge
					if (!edgesWithinCluster.containsKey(clusterA)) {
						...
						edgesWithinCluster.put(clusterA, edgesWithinCluster.get(clusterA) + 1);
	...
	public double calculateLocalClustCoef() {
		...
		double clust_coef = 0.0;
		double total_clust_coef = 0.0;
		Vertex vertex;
		Iterator<Vertex> itVertex = net.getVertexIterator();
		while (itVertex.hasNext()) {
			vertex = itVertex.next();
		...
		return total_clust_coef / net.getNumVertices();
	}
